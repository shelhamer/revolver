...
class SlugFilter(Wrapper, Dataset):
    """
    Filter a dataset based on information in the auxiliary dict

    Args:
        dataset: the `Dataset` to filter (which must be a `SegData`).
        key: key to access auxiliary dict
        keep: list of values to keep
    """

    def __init__(self, dataset, key, keep):
        super().__init__(dataset)
        self.ds = dataset
        self.key = key
        self.keep = keep
    ...
    def load_slugs(self):
        ...
        for i, data in enumerate(self.ds):
            aux = data[-1]
            if self.key not in aux:
                raise Exception('Cannot filter by the given key - it is not in the aux dict')
            if aux[self.key] in self.keep:
                slugs.append(i)
        return slugs
