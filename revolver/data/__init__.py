...
from .pascal import *
from .davis import DAVISInstSeg
from .seg import MaskInstSeg, MaskSemSeg
from .sparse import SparseSeg
from .interactive import InteractiveSeg
from .filter import TargetFilter, TargetMapper, SubSampler, MultiFilter, SlugFilter
from .conditional import ConditionalInstSeg, ConditionalSemSeg, ConditionalVideoSeg
...
def prepare_interactive_video_data(dataset_name, split, classes_to_filter=None, count=None, shot=None, multi=False):
    cache_path = make_cache_path('interactive-video', dataset_name, split, classes_to_filter, count, multi)
    if os.path.isfile(cache_path):
    ...
    if multi:
        sem_ds = MultiFilter(sem_ds)
    sparse_ds = sparsify(sem_ds, count)
    inter_ds = InteractiveSeg(sem_ds, sparse_ds)
    image_transform = Compose([
        ImToCaffe(mean=sem_ds.mean),
        NpToTensor()
    ])
...
def prepare_early_conditional_video_data(dataset_name, split, classes_to_filter=None, count=None, shot=None, multi=False):
    cache_path = make_cache_path('early-conditional-video', dataset_name, split, classes_to_filter, count, multi)
    if os.path.isfile(cache_path):
        ds = pickle.load(open(cache_path, 'rb'))
        ds.ds.shot = shot  # don't worry about it...
        return ds
    is_eval = ('val' in split)
    sem_dataset = datasets[dataset_name]
    sem_ds = sem_dataset(split=split)
    if multi:
        sem_ds = MultiFilter(sem_ds)
    vid_dsets = {vid: sparsify(SlugFilter(sem_ds, 'vid', [vid]), count=count, static=is_eval) for vid in sem_ds.load_videos()}
...
def prepare_late_conditional_video_data(dataset_name, split, classes_to_filter=None, count=None, shot=None, multi=False):
    cache_path = make_cache_path('late-conditional-video', dataset_name, split, classes_to_filter, count, multi)
    if os.path.isfile(cache_path):
        ds = pickle.load(open(cache_path, 'rb'))
        ds.ds.shot = shot  # don't worry about it...
        return ds
...
def sparsify(ds, count=None, static=False):
    if count is None:
        count = list(range(100))
    return SparseSeg(ds, count=count, static=static)
...
datatypes = {
    ...
    'interactive-video': prepare_interactive_video_data,
    ...
    'early-conditional-video': prepare_early_conditional_video_data,
    'late-conditional-video': prepare_late_conditional_video_data,
}
